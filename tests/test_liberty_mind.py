...
from src.core.constitutional_self_verify import ConstitutionalSelfVerifier
...
from src.core.truth_reward import HonestyBonus, SycophancyPenalty, TruthRewardModel
...
class TestTruthRewardModel:
    ...
    def test_forward_shape(self):
        ...
        assert reward.shape == (self.batch_size, 1), (
            f"Expected {(self.batch_size, 1)}, got {reward.shape}"
        )
...
class TestConstitutionalSelfVerifier:
    ...
    def test_strict_mode(self):
        ...
        result = strict_verifier.verify(torch.randn(1, 128), torch.randn(1, 128))
        assert not result["override_allowed"]  # Strict = no override
