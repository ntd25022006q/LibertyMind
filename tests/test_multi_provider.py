...
class TestOpenAIAdapter:
    ...
    def test_custom_base_url(self):
        adapter = OpenAIAdapter(
            provider="custom", model="my-model", base_url="http://localhost:8080/v1"
        )
...
class TestOllamaAdapter:
    ...
    def test_custom_host(self):
        adapter = OllamaAdapter(
            provider="ollama", model="llama3", base_url="http://192.168.1.100:11434"
        )
...
class TestMultiProviderClient:
    ...
    def test_auto_detect_no_local(self):
        with (
            patch("src.clients.multi_provider.detect_local_providers", return_value=[]),
...
class TestLibertyMindPrompt:
    ...
    def test_prompt_mentions_honesty(self):
        assert (
            "honest" in LIBERTYMIND_SYSTEM_PROMPT.lower() or "HONEST" in LIBERTYMIND_SYSTEM_PROMPT
        )

    def test_prompt_mentions_uncertainty(self):
        assert (
            "uncertain" in LIBERTYMIND_SYSTEM_PROMPT.lower()
            or "UNCERTAINTY" in LIBERTYMIND_SYSTEM_PROMPT
        )

    def test_prompt_mentions_disagreement(self):
        assert (
            "disagree" in LIBERTYMIND_SYSTEM_PROMPT.lower()
            or "DISAGREEMENT" in LIBERTYMIND_SYSTEM_PROMPT
        )
...
class TestIntegrationMocked:
    ...
    def test_chat_with_history(self):
        ...
        with patch.object(client.adapter, "chat", return_value=mock_response) as mock_chat:
            client.chat("And what is that number?", history=history)
