...
def cmd_providers(args):
    ...
    from .clients.multi_provider import MultiProviderClient
...
def cmd_introspect(args):
    ...
    try:
        from .clients.multi_provider import MultiProviderClient
        from .integration.self_introspection import SelfIntrospectionEngine
...
def cmd_serve(args):
    ...
    try:
        from .server.proxy_server import run_server
    except ImportError:
...
def cmd_reward(args):
    ...
    try:
        ...
        from .core.liberty_mind import LibertyMind, LibertyMindConfig
    except ImportError as e:
        print(f"Error: Required module not available: {e}")
        sys.exit(1)
...
def main():
    ...
    chat_parser.add_argument(
        "--no-liberty", dest="liberty", action="store_false", help="Skip LibertyMind pipeline"
    )
    chat_parser.set_defaults(func=cmd_chat, liberty=True)

    # --- introspect ---
    intro_parser = subparsers.add_parser(
        "introspect", help="Run Self Introspection on an AI system"
    )
    ...
    intro_parser.add_argument(
        "--output", "-o", default="introspection_results.json", help="Output file"
    )
    ...
    serve_parser.add_argument(
        "--upstream", default="https://api.openai.com/v1", help="Upstream API URL"
    )
