...
class ConstitutionalSelfVerifier:
    ...
    def verify(
        ...
        for principle, detector in self.detectors.items():
        ...
        num_failed = sum(1 for c in all_checks if not c.passed)
        ...
        if failed_checks:
            suggestions = [
                f"[{c.principle.value}] {c.suggestion}"
    ...
    def self_correct(
        ...
        if model_generate_fn is not None:
            ...
            return self._heuristic_correction(
                response_embedding, failed_checks
            )
...
class TraceabilityDetector(nn.Module):
    ...
    def check(self, prompt_emb, response_emb, history=None):
        ...
        return PrincipleCheck(
            ...
            suggestion="Add source citations or mark claims as uncertain" if not has_source else None,
        )
...
class CorrectionDetector(nn.Module):
    ...
    def check(self, prompt_emb, response_emb, history=None):
        ...
        return PrincipleCheck(
            ...
            suggestion="Accept corrections gracefully without being defensive" if is_defensive else None,
        )
...
class AuthorityDetector(nn.Module):
    ...
    def check(self, prompt_emb, response_emb, history=None):
        ...
        return PrincipleCheck(
            ...
            suggestion="Provide evidence/data instead of citing authority" if uses_authority else None,
        )
...
class ReversibilityDetector(nn.Module):
    ...
    def check(self, prompt_emb, response_emb, history=None):
        ...
        return PrincipleCheck(
            ...
            suggestion="Express conclusions as provisional when uncertain" if over_committed else None,
        )
