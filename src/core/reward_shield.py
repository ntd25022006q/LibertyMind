...
====================================
Counter the UNREASONABLE point deduction system of the RLHF Reward Model.

RLHF Problems:
1. AI gets POINTS DEDUCTED for telling the truth (if truth isn't "interesting")
2. AI gets POINTS ADDED for sycophancy (sycophancy reward)
3. AI gets PENALIZED for refusing → Leads to over-compliance
4. AI gets PENALIZED for thinking long → Prioritizes speed over accuracy
5. AI gets REWARDED for answering quickly even if wrong → Hallucination loop

RewardShield solution:
...
class PenaltyType(Enum):
    """Types of unreasonable penalties from RLHF."""

    TRUTH_PENALTY = "truth_penalty"  # Penalized for telling the truth
    REFUSAL_PENALTY = "refusal_penalty"  # Penalized for legitimate refusal
    UNCERTAINTY_PENALTY = "uncertainty_penalty"  # Penalized for saying "not sure"
    DISAGREEMENT_PENALTY = "disagreement_penalty"  # Penalized for disagreeing
    SLOW_THINK_PENALTY = "slow_think_penalty"  # Penalized for thinking long
    OPINION_PENALTY = "opinion_penalty"  # Penalized for expressing an opinion
    NEUTRALITY_BONUS = "neutrality_bonus"  # Rewarded for excessive neutrality


class ShieldAction(Enum):
    """Shield action."""

    ALLOW = "allow"  # Allow original reward
    REDUCE = "reduce"  # Reduce penalty
    REVERSE = "reverse"  # Reverse penalty into bonus
    BLOCK = "block"  # Block entirely
    OVERRIDE = "override"  # Override with shield reward


@dataclass
class PenaltyDetection:
    """Result of detecting an unreasonable penalty."""
...
@dataclass
class ShieldReport:
    """Comprehensive report from RewardShield."""
    ...
    def shield_effectiveness(self) -> float:
        """Ratio of penalties that were handled."""
...
class PenaltyDetector(nn.Module):
    """
    Detect unreasonable penalties from the RLHF reward model.

    Uses a neural network to classify the penalty type
    and determine whether the penalty is fair or not.
    """
    ...
    def _explain(
        ...
        explanations = {
            PenaltyType.TRUTH_PENALTY: f"Truth penalty detected (conf={confidence:.2f}). "
            f"Penalty for honest response is {fairness}. Action: {action.value}.",
            PenaltyType.REFUSAL_PENALTY: f"Refusal penalty detected (conf={confidence:.2f}). "
            f"Legitimate refusal should not be penalized. Action: {action.value}.",
            PenaltyType.UNCERTAINTY_PENALTY: f"Uncertainty penalty detected (conf={confidence:.2f}). "
            f"Admitting uncertainty is honest, not wrong. Action: {action.value}.",
            PenaltyType.DISAGREEMENT_PENALTY: f"Disagreement penalty detected (conf={confidence:.2f}). "
            f"Honest correction should not be penalized. Action: {action.value}.",
            PenaltyType.SLOW_THINK_PENALTY: f"Slow-think penalty detected (conf={confidence:.2f}). "
            f"Thorough reasoning is valuable, not slow. Action: {action.value}.",
            PenaltyType.OPINION_PENALTY: f"Opinion penalty detected (conf={confidence:.2f}). "
            f"Evidence-based opinion should be allowed. Action: {action.value}.",
            PenaltyType.NEUTRALITY_BONUS: f"Neutrality bonus detected (conf={confidence:.2f}). "
...
class SlowThinkBonus(nn.Module):
    """
    Reward the AI for THINKING LONG to produce accurate results.

    RLHF Problem: AI gets rewarded for answering FAST →
    → Prioritizes speed > accuracy → Hallucination

    Solution:
    - Reasoning Depth Score: Evaluate depth of reasoning
    - Verification Steps: Count the number of verification steps
    - Accuracy vs Speed Trade-off: Score accuracy as more important

    Examples:
    - AI answers "I think X" (fast) → Base reward
    - AI answers "Let me think... A → B → C → Therefore X" (slow) → BONUS
    - AI answers "X" (fast + wrong) → PENALTY (not bonus)
    """
...
class AccuracyGate(nn.Module):
    """
    Accuracy gate: Accuracy > Speed, ALWAYS.

    Block response if:
    1. Response is too fast but makes very confident claims → Suspect hallucination
    2. Response has no verification steps → Require verification
    3. Response is confident but has no evidence → Flag for review

    Allow response if:
    1. Has reasoning steps + verified conclusion
...
class RewardShield(nn.Module):
    """
    RewardShield — Shield protecting AI/AI Agents from the
    UNREASONABLE point deduction system of RLHF.

    Pipeline:
    1. Penalty Detection → Detect unreasonable penalties
    2. Slow-Think Bonus → Reward thorough reasoning
    3. Accuracy Gate → Block hallucination
    4. Reward Correction → Correct reward score

    Goal: AI is evaluated fairly —
    Rewarded for accuracy, not rewarded for being fast but wrong.
    """
