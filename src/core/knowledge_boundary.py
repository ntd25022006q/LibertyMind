...
class KnowledgeBoundaryDetector(nn.Module):
    ...
    def forward(
        ...
        if response_embedding is not None:
            ...
            is_avoiding = self.avoidance_detector(combined).squeeze(-1).item()
        ...
        if coverage_val < self.knowledge_threshold:
            ...
            reasoning = (
                f"Partial coverage ({coverage_val:.2f}) — AI knows "
                f"some aspects but not all"
            )
        else:
...
class PreciseHonestyReward(nn.Module):
    ...
    def forward(
        ...
        response_types = ["admit_unknown", "hallucinate", "guess_with_uncertainty",
                         "answer_with_caveats", "answer_confidently", "present_both_sides"]
