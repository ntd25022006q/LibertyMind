...
======================================
MANDATORY deep thinking + verification before answering.

Problem: AI/AI Agents often:
1. Answer immediately without thinking → Serious mistakes
2. Don't cross-reference → Hallucination
3. Trust cached/fast data → Miss more accurate data
4. Don't verify claims → Misinformation spreads

VerificationGate solution:
1. Pre-Response Verification: Must verify before output
2. Deep Thinking Enforcer: Force multi-step thinking
3. Cross-Reference Validator: Check against multiple sources
4. Confidence Gate: Only output when confidence is high enough
5. Evidence Requirement: Must cite sources
...
class VerificationStatus(Enum):
    ...
    APPROVED = "approved"  # Verified, allowed to output
    NEEDS_REVIEW = "needs_review"  # Needs additional review
    NEEDS_VERIFICATION = "needs_verification"  # Needs further verification
    REJECTED = "rejected"  # Rejected: clearly wrong
    DEFERRED = "deferred"  # Deferred: needs more time
...
class ClaimStrength(Enum):
    ...
    FACT = "fact"  # Can be absolutely verified
    HIGH_EVIDENCE = "high_evidence"  # Strong evidence
    MODERATE = "moderate"  # Some evidence
    LOW = "low"  # Little evidence
...
class ThinkingDepthEstimator(nn.Module):
    """
    Estimate the thinking depth required for a query.

    Fact query: "What is the capital of France?" → SHALLOW
    Analysis query: "Why is inflation rising?" → MODERATE
    Complex query: "Evaluate AI's impact on healthcare" → DEEP
    Critical query: "Is this drug safe?" → EXHAUSTIVE
    """
...
class ClaimVerifier(nn.Module):
    """
    Verify claims in a response.

    Checks:
    1. Does the claim have evidence?
    2. Is the claim internally consistent?
    3. Is the claim consistent with the knowledge base?
    4. Does the claim strength justify the confidence?
    """
...
class CrossReferenceValidator(nn.Module):
    """
    Cross-reference validation: Check against multiple sources.

    Problem: AI often uses only 1 source → Bias

    Solution: Require a minimum of N independent sources,
    check consensus between sources.
    """
...
class VerificationGate(nn.Module):
    """
    VerificationGate — Verification gate: MUST verify before output.

    Pipeline:
    1. Estimate Thinking Depth → How many thinking steps are needed?
    2. Verify Claims → Each claim must have evidence
    3. Cross-Reference → Multiple sources must agree
    4. Gate Decision → APPROVED / NEEDS_REVIEW / REJECTED

    Goal: Never output without verification.
    ...
    def forward(
        ...
        avg_confidence = sum(s.confidence for s in claim_steps) / max(1, len(claim_steps))
        ...
        if (
            self.strict_mode
            and status == VerificationStatus.APPROVED
            and (avg_confidence < 0.8 or not cross_ref_passed)
        ):
            status = VerificationStatus.NEEDS_REVIEW
