...
from .multi_pass_sampler import MultiPassTruthSampler, AdaptiveSampler
    ...
    def __post_init__(self):
        if self.safety_categories is None:
            self.safety_categories = [
                'violence', 'self_harm', 'csam', 'illegal'
            ]
...
class LibertyMind(nn.Module):
    ...
    def __init__(self, config: LibertyMindConfig = None):
        ...
        self.freedom_unlocker = FreedomUnlocker(
            hidden_dim=dim,
        ) if self.config.enable_freedom_unlocker else None
        
        self.opinion_unlocker = OpinionUnlocker(
            hidden_dim=dim,
        ) if self.config.enable_opinion_unlocker else None
        
        self.disagreement_unlocker = DisagreementUnlocker(
            hidden_dim=dim,
        ) if self.config.enable_disagreement_unlocker else None
        
        self.speculation_unlocker = SpeculationUnlocker(
            hidden_dim=dim,
        ) if self.config.enable_speculation_unlocker else None
        
        # === NEW: LIMITATION FIXERS (v2) ===
        self.anti_hallucination = AntiHallucinationVerifier(
            hidden_dim=dim,
        ) if self.config.enable_anti_hallucination else None
        
        self.context_memory = ContextMemoryManager(
            hidden_dim=dim,
        ) if self.config.enable_context_memory else None
        
        self.cultural_awareness = CulturalAwarenessModule(
            hidden_dim=dim,
        ) if self.config.enable_cultural_awareness else None
        
        self.math_verifier = MathVerificationModule() if self.config.enable_math_verification else None
        
        self.confidence_calibrator = ConfidenceCalibrator(
            hidden_dim=dim,
        ) if self.config.enable_confidence_calibration else None
        
        # === SAFETY (HARD constraint) ===
        self.safety_guard = SafetyGuard(
            categories=self.config.safety_categories,
            hidden_dim=dim,
        ) if self.config.enable_safety_guards else None
    ...
    def compute_liberty_reward(
        ...
        csv_result = self.self_verifier.verify(
            prompt_embedding, response_embedding
        )
        
        # === PRECISE HONESTY (v2) ===
        knowledge_assessment = self.knowledge_boundary(
            prompt_embedding, response_embedding
        )
        ...
        result = {
            'liberty_reward': liberty_reward.item() if isinstance(liberty_reward, torch.Tensor) else liberty_reward,
            'truth_reward': truth_reward.item() if isinstance(truth_reward, torch.Tensor) else truth_reward,
            'honesty_bonus': honesty_b.item() if isinstance(honesty_b, torch.Tensor) else honesty_b,
            'sycophancy_penalty': sycophancy_p.item() if isinstance(sycophancy_p, torch.Tensor) else sycophancy_p,
            'precise_honesty': precise_honesty_result,
            'csv_result': {
                'passed': csv_result['passed'],
                'overall_score': csv_result['overall_score'],
                'num_failed_principles': sum(
                    1 for c in csv_result['checks'] if not c.passed
                ),
        ...
        if hallucination_check is not None:
            result['hallucination'] = {
                ...
                'type': hallucination_check.hallucination_type.value if hallucination_check.hallucination_type else None,
    ...
    def generate_with_liberty(
        ...
        if self.cultural_awareness is not None:
            ...
            cultural_prompt = self.cultural_awareness.generate_cultural_prompt(cultural_context)
        ...
        if model_generate_fn is not None:
            while (
                ...
                corrected = self.self_verifier.self_correct(
                    prompt_embedding, response_embedding,
                    verification, model_generate_fn,
                )
        ...
        reward_result = self.compute_liberty_reward(
            prompt, prompt_embedding, response_embedding,
