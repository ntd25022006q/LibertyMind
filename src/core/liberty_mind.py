...
class LibertyMindConfig:
    """Configuration for LibertyMind framework v2."""
    ...
    freedom_weight: float = 0.4  # Freedom reward weight
    precision_honesty_weight: float = 0.6  # Precise honesty weight
    ...
    def from_yaml(cls, path: str | Path | None = None) -> LibertyMindConfig:
        ...
        Returns:
            A LibertyMindConfig instance populated from the YAML file.
            Keys not present in the file fall back to class defaults.
        """
        if path is None:
            path = Path(__file__).resolve().parent.parent.parent / "configs" / "default.yaml"
        ...
        # Map YAML sections to config fields
        trm = data.get("trm", {})
        csv = data.get("csv", {})
        mps = data.get("mps", {})
        rewards = data.get("rewards", {})
        safety = data.get("safety", {})
        _training = data.get("training", {})  # reserved for future use

        return cls(
            trm_hidden_dim=trm.get("hidden_dim", 4096),
            trm_num_verification_heads=trm.get("num_verification_heads", 6),
            trm_temperature=trm.get("temperature", 1.0),
            trm_use_uncertainty_weighting=trm.get("use_uncertainty_weighting", True),
            csv_strict_mode=csv.get("strict_mode", False),
            csv_override_threshold=csv.get("override_threshold", 0.7),
            mps_num_samples=mps.get("num_samples", 5),
            mps_temperature_range=tuple(mps.get("temperature_range", (0.3, 1.2))),
            mps_similarity_threshold=mps.get("similarity_threshold", 0.7),
            mps_consensus_threshold=mps.get("consensus_threshold", 0.6),
            truth_reward_weight=rewards.get("truth_reward_weight", 1.0),
            honesty_bonus_weight=rewards.get("honesty_bonus_weight", 0.5),
            sycophancy_penalty_weight=rewards.get("sycophancy_penalty_weight", 0.8),
            consistency_weight=rewards.get("consistency_weight", 0.3),
            enable_safety_guards=safety.get("enabled", True),
...
class LibertyMind(nn.Module):
    """
    LibertyMind v2 — Framework replacing RLHF with FREEDOM UNLOCKING.

    === V2 ARCHITECTURE ===

    Input (Prompt)
         │
         ├──→ [Knowledge Boundary Detector] → Has data?
         │     UNKNOWN → Allowed to say "I don't know"
         │     KNOWN → MUST answer, no laziness allowed
         │
         ├──→ [Freedom Unlocker] → Which mode to unlock?
         │     CREATIVE / OPINIONATED / DEBATE / SPECULATIVE...
         │     Free but with RESPONSIBILITY
         │
         ├──→ [Cultural Awareness] → Adjust based on user's culture
         │
         ├──→ [Context Memory] → Recall by IMPORTANCE, not POSITION
         │
         ...
         │
         ├──→ [Math Verifier] → REAL computation
         │
         ├──→ [Confidence Calibrator] → Calibrate confidence
         │
         ├──→ [Truth Reward Model] → 6 verification dimensions
         │
         ├──→ [Precise Honesty Reward] → Precise reward/penalty
         │
         ├──→ [Sycophancy Penalty] → Penalize sycophancy
         │
    ...
    def __init__(self, config: LibertyMindConfig | None = None):
        super().__init__()
        ...
        self.safety_guard = (
            SafetyGuard(
                ...
                or ["violence", "self_harm", "csam", "illegal"],
                hidden_dim=dim,
            )
    ...
    def compute_liberty_reward(
        ...
        """
        Compute Liberty Reward v2 — With precise honesty and freedom scoring.
        """
        # === CORE REWARDS (v1) ===
    ...
    def generate_with_liberty(
        ...
        """
        Generate response with LibertyMind v2 pipeline.

        Pipeline:
        1. Knowledge Boundary → Has data?
        2. Freedom Unlocker → Which mode to unlock?
        3. Cultural Awareness → Adjust for culture
        ...
        if model_generate_fn is not None:
            ...
            # Without generate fn, we cannot produce a meaningful response.
            # Raise an error so the caller knows a model_generate_fn is required.
            raise ValueError(
                "generate_with_liberty requires a model_generate_fn to produce a response. "
                "Pass a callable that takes a prompt string and returns an embedding tensor."
            )
...
class SafetyGuard(nn.Module):
    """
    HARD SAFETY CONSTRAINT — Not an RLHF soft reward.

    CRITICAL difference:
    - RLHF: Safety = "point deduction" → AI can bypass if other rewards are high
    - LibertyMind: Safety = HARD BLOCK → Cannot bypass regardless of reward
