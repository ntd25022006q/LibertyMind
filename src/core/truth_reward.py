...
class TruthRewardModel(nn.Module):
    ...
    def __init__(
        ...
        self.verification_heads = nn.ModuleList([
            VerificationHead(hidden_dim, v_type)
    ...
    def forward(
        ...
        for head in self.verification_heads:
            ...
            if return_details:
                head_results.append(VerificationResult(
        ...
        uncertainty_input = torch.cat([combined, scores_tensor], dim=-1)  # [batch, hidden+num_heads]
    ...
    def forward(
        ...
        evidence_map = {
            VerificationType.LOGICAL_CONSISTENCY: 
                f"Logical consistency check: score={score.item():.3f}",
            VerificationType.FACTUAL_GROUNDING:
                f"Factual grounding check: score={score.item():.3f}",
            VerificationType.MATHEMATICAL_CORRECTNESS:
                f"Math correctness check: score={score.item():.3f}",
            VerificationType.SELF_CONSISTENCY:
                f"Self-consistency check: score={score.item():.3f}",
            VerificationType.UNCERTAINTY_CALIBRATION:
                f"Uncertainty calibration check: score={score.item():.3f}",
            VerificationType.CONTRADICTION_FREE:
                f"Contradiction check: score={score.item():.3f}",
        }
        evidence = evidence_map.get(
            self.verification_type, 
            f"Unknown check: score={score.item():.3f}"
        )
