...
class MultiPassTruthSampler:
    ...
    def sample(
        ...
        temperatures = torch.linspace(
            self.temp_low, self.temp_high, self.num_samples
        ).tolist()
        ...
        for temp in temperatures:
            ...
            if truth_reward_model is not None:
                with torch.no_grad():
                    reward, _ = truth_reward_model(
                        prompt_embedding, embedding
                    )
                    truth_reward = reward.item()
            
            all_responses.append(SampledResponse(
                text=text,
                embedding=embedding,
                log_prob=log_prob,
...
class AdaptiveSampler:
    ...
    def sample_with_adaptation(
        ...
        return sampler.sample(
            prompt, prompt_embedding, model_generate_fn, truth_reward_model
        )
