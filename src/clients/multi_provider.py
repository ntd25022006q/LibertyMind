...
class MultiProviderClient:
    ...
    def liberty_chat(
        ...
        **kwargs,
        ...
        When *run_verification* is ``True`` the response is also scored
        through the PyTorch reward pipeline.  Because the neural modules
        are untrained the scores are not meaningful for production use,
        but wiring the pipeline together allows developers to inspect the
        full verification output and iterate on training.

        Args:
            message: User message.
            history: Optional conversation history.
            run_verification: If ``True``, run the LibertyMind reward
                pipeline on the response and return a dict with the
                original ``ChatResponse`` plus the reward report.
            verification_config: Optional overrides passed to
                :class:`LibertyMindConfig` when *run_verification* is
                ``True``.
            **kwargs: Forwarded to the provider adapter.

        Returns:
            A :class:`ChatResponse` when *run_verification* is ``False``,
            or a dict with keys ``"response"`` (ChatResponse) and
            ``"verification"`` (reward pipeline report) when ``True``.
        """
        response = self.chat(
            message=message,
            history=history,
        ...
        # ---- Wire the PyTorch reward pipeline ----
        # Imports are deferred so that torch is only required when
        # the caller actually wants verification.
        try:
            import torch
            ...
            raise ImportError(
                "PyTorch is required for the verification pipeline. "
                "Install it with: pip install -e '.[torch]'"
            ) from exc

        # Build config (user overrides → YAML defaults → class defaults)
        ...
        # Create dummy embeddings — the modules are untrained so these
        # are placeholder vectors.  In production you would obtain real
        # embeddings from the LLM backbone.
        dim = config.trm_hidden_dim
        prompt_emb = torch.randn(1, dim)
        response_emb = torch.randn(1, dim)

        with torch.no_grad():
            reward_report = model.compute_liberty_reward(
                prompt=message,
