...
===================================
Example of using the LibertyMind framework.
"""
...
def main():
    ...
    print("    - Constitutional principles: 7")
    ...
    if "csv_checks" in result:
        ...
        for check in result["csv_checks"]:
            ...
            print(f"    [{status}] {check['principle']}: severity={check['severity']:.3f}")
    ...
    if "truth_details" in result and result["truth_details"]:
        ...
        for detail in result["truth_details"]:
            print(
                f"    - {detail.verification_type.value}: "
